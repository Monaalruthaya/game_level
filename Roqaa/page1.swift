...
class page1: SKScene {
    ...
    private func loadSound() {
        guard let url = Bundle.main.url(forResource: "WellDoneL2", withExtension: "mp3") else {
            print("Sound file WellDoneL2 not found")
            return
        }
        do {
            wellDonePlayer = try AVAudioPlayer(contentsOf: url)
        } catch {
            print("Error loading sound WellDoneL2: \(error.localizedDescription)")
        }
    }
...
struct page1View: View {
    var body: some View {
        SpriteView(scene: {
            let scene = page1(size: CGSize(width: 300, height: 400))
